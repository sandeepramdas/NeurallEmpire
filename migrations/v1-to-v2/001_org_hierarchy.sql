...
ADD COLUMN IF NOT EXISTS "parentId" TEXT REFERENCES "Organization"("id") ON DELETE RESTRICT;
...
ON "Organization" USING GIN("ancestorIds");
...
ON "Organization"("deletedAt", "level")
...
WITH RECURSIVE hierarchy AS (
    ...
    SELECT
        ...
        h.path || o.id as path
    FROM hierarchy h
    JOIN "Organization" o ON o."parentId" = h."descendantId"
...
BEGIN
    ...
    JOIN "Organization" o ON o.id = oc."descendantId"
...
BEGIN
    ...
    FROM "OrganizationClosure" oc
    JOIN "Organization" o ON o.id = oc."ancestorId"
...
-- Drop triggers
DROP TRIGGER IF EXISTS "trigger_refresh_org_closure" ON "Organization";
...
-- Drop constraints
ALTER TABLE "Organization" DROP CONSTRAINT IF EXISTS "check_no_self_parent";

-- Drop columns (DANGEROUS - only if no data exists)
-- ALTER TABLE "Organization" DROP COLUMN IF EXISTS "hierarchyPosition";
-- ALTER TABLE "Organization" DROP COLUMN IF EXISTS "type";
-- ALTER TABLE "Organization" DROP COLUMN IF EXISTS "ancestorIds";
-- ALTER TABLE "Organization" DROP COLUMN IF EXISTS "level";
-- ALTER TABLE "Organization" DROP COLUMN IF EXISTS "path";
-- ALTER TABLE "Organization" DROP COLUMN IF EXISTS "parentId";
...
INSERT INTO "Organization" (id, name, slug, "planType")
VALUES
    ('org1', 'Parent Corp', 'parent-corp', 'ENTERPRISE'),
    ('org2', 'Subsidiary A', 'subsidiary-a', 'ENTERPRISE'),
    ('org3', 'Division B', 'division-b', 'PROFESSIONAL');

UPDATE "Organization" SET "parentId" = 'org1', level = 1, path = 'org1.org2'
WHERE id = 'org2';

UPDATE "Organization" SET "parentId" = 'org2', level = 2, path = 'org1.org2.org3'
WHERE id = 'org3';
