...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const queries = await (prisma as any).connectorQuery.findMany({
      where: {
    ...
    const total = await (prisma as any).connectorQuery.count({
      where: {
        connectorId: req.params.id,
      },
    });
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const logs = await (prisma as any).connectorAuditLog.findMany({
      where: {
        connectorId: req.params.id,
      },
      take: parseInt(limit as string),
    ...
    const total = await (prisma as any).connectorAuditLog.count({
      where: {
        connectorId: req.params.id,
      },
    });
