...
export class ConnectorService {
  ...
  async getConnectorStats(connectorId: string, organizationId: string) {
    ...
    // Get database stats
    const queryCount = await prisma.connectorQuery.count({
      where: { connectorId },
    });

    const errorCount = await prisma.connectorQuery.count({
      where: {
        connectorId,
        status: 'FAILED',
      },
    ...
    const avgDuration = await prisma.connectorQuery.aggregate({
      where: { connectorId },
