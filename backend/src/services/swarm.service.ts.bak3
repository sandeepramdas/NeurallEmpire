...
class SwarmService extends EventEmitter {
    ...
    try {
      ...
      const member = await prisma.swarmMember.create({
        data: {
          ...
          priority: memberConfig.priority,
        },
      });
    ...
    try {
      ...
      switch (swarm.coordinatorType) {
        ...
        case SwarmType.PARALLEL:
          ...
          break;
        case SwarmType.CONDITIONAL:
          result = await this.executeConditional(swarm, context);
          break;
        ...
        case SwarmType.HIERARCHICAL:
          ...
          break;
        default:
          throw new Error(`Unsupported swarm type: ${swarm.coordinatorType}`);
      }
  ...
  private async executeConditional(swarm: any, context: SwarmExecutionContext): Promise<any> {
    const results: any[] = [];
    const config = swarm.configuration || {};

    // Sort by priority for conditional execution
    const sortedMembers = swarm.members.sort((a: any, b: any) => b.priority - a.priority);
    ...
    for (const member of sortedMembers) {
      ...
      // Check if conditions are met for this agent
      const shouldExecute = await this.evaluateConditions(member, context, results);

      if (shouldExecute) {
        try {
          console.log(`Executing agent ${member.agentId} (${member.role}) - conditions met`);
          ...
          // Update shared data
          if (agentResult.success && agentResult.output?.sharedData) {
            Object.entries(agentResult.output.sharedData).forEach(([key, value]) => {
              context.sharedData!.set(key, value);
            });
          }
          ...
          console.error(`Agent ${member.agentId} failed in conditional execution:`, error);
          results.push({
            agentId: member.agentId,
            role: member.role,
            error: error instanceof Error ? error.message : 'Unknown error',
        ...
        console.log(`Skipping agent ${member.agentId} - conditions not met`);
        results.push({
          agentId: member.agentId,
          role: member.role,
          skipped: true,
  ...
  private async executeCollaborative(swarm: any, context: SwarmExecutionContext): Promise<any> {
    ...
    const observers = swarm.members.filter((m: any) => m.role === SwarmRole.OBSERVER);
    ...
    // Phase 3: Observers monitor and validate
    if (observers.length > 0) {
      for (const observer of observers) {
        try {
          const observeResult = await agentService.executeAgent(observer.agentId, {
            phase: 'observation',
            workerResults: results.filter(r => r.role === SwarmRole.WORKER),
            swarmContext: {
              swarmId: context.swarmId,
              executionId: context.executionId,
              role: 'observer',
          ...
          console.error(`Observer ${observer.agentId} failed:`, error);
        }
      }
    }
