...
const Webhooks: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 dark:text-gray-100">Webhooks</h1>
        <p className="text-gray-600 dark:text-gray-400 dark:text-gray-400 mt-2">Manage webhook endpoints and delivery logs</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">Total Webhooks</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-1">{totalWebhooks}</p>
            </div>
            <Webhook className="w-8 h-8 icon-active" />
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">Total Deliveries</p>
              <p className="text-2xl font-bold text-blue-600 mt-1">{totalDeliveries.toLocaleString()}</p>
            </div>
            <Send className="w-8 h-8 icon-active" />
          </div>
        </div>
      ...
      <div className="space-y-4">
        {webhooks.length === 0 ? (
          ...
          webhooks.map((webhook) => (
            <div key={webhook.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
              {/* Webhook Header */}
              <div className="p-6 border-b border-gray-200 dark:border-gray-700">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <div>
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 dark:text-gray-100">{webhook.name}</h3>
                        <div className="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">
                ...
                <div className="mt-4 grid grid-cols-3 gap-4">
                  <div className="text-center p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                    <p className="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">Total</p>
                    <p className="text-lg font-bold text-gray-900 dark:text-gray-100 dark:text-gray-100">{webhook.deliveryStats.total}</p>
                  </div>
                  <div className="text-center p-3 bg-green-50 rounded-lg">
                    <p className="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">Successful</p>
                    <p className="text-lg font-bold text-green-600">{webhook.deliveryStats.successful}</p>
                  </div>
                  <div className="text-center p-3 bg-red-50 rounded-lg">
                    <p className="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">Failed</p>
              ...
              <div className="p-6">
                ...
                <div className="space-y-2">
                  {getWebhookLogs(webhook.id).length === 0 ? (
                    <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-4">No delivery logs yet</p>
                  ) : (
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            {log.status === 'success' ? (
                              ...
                              <Clock className="w-5 h-5 text-gray-600 dark:text-gray-400 dark:text-gray-400" />
                            )}
                            <div>
                              <div className="flex items-center space-x-2">
                                <span className="text-sm font-medium text-gray-900 dark:text-gray-100 dark:text-gray-100">{log.event}</span>
                          ...
                          {log.status === 'failed' && (
                            <button
                              ...
                              className="px-3 py-1 text-sm text-blue-600 hover:bg-blue-50 dark:bg-blue-900/20 rounded transition-colors flex items-center"
                            >
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 dark:text-gray-100 flex items-center">
                <Webhook className="w-6 h-6 mr-2 icon-active" />
                Create Webhook
              </h2>
              <button
            ...
            <form onSubmit={handleCreateWebhook} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Webhook Name</label>
                <input
                  type="text"
                  value={webhookName}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto p-3 border border-gray-300 dark:border-gray-600 rounded-lg">
                  {eventOptions.map((event) => (
                    <label key={event.value} className="flex items-center space-x-2 cursor-pointer">
                      ...
                      />
                      <span className="text-sm text-gray-700 dark:text-gray-300 dark:text-gray-300">{event.label}</span>
                    </label>
                  ))}
                </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Custom Headers (Optional)</label>
                {customHeaders.map((header, index) => (
                  <div key={index} className="flex items-center space-x-2 mb-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm"
                    />
                    <input
                      type="text"
                      value={header.value}
                ...
                <button
                  ...
                  className="text-sm text-indigo-600 hover:opacity-80 flex items-center"
                >
      ...
      {isPayloadModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 dark:text-gray-100 flex items-center">
                <Code className="w-6 h-6 mr-2 icon-active" />
            ...
            <div className="space-y-4">
              <div>
                <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-2">Example Payload Structure</h3>
              ...
              <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-3">Authentication</h3>
              ...
              <div>
                <a
                  ...
                  className="text-indigo-600 hover:opacity-80 text-sm flex items-center"
                >
